...
try{if($('.datanew').length>0){$('.datanew').DataTable({"bFilter":true,"sDom":'fBtlpi','pagingType':'numbers',"ordering":true,"language":{search:' ',sLengthMenu:'_MENU_',searchPlaceholder:"Search...",info:"_START_ - _END_ of _TOTAL_ items",},initComplete:(settings,json)=>{$('.dataTables_filter').appendTo('#tableSearch');$('.dataTables_filter').appendTo('.search-input');},});}}catch(e){console.error('DataTable init (.datanew) a échoué, le reste de la page continue :',e);}
...
$("#imgInp").change(function(){readURL(this);});try{if($('.datatable').length>0){$('.datatable').DataTable({"bFilter":false});}}catch(e){console.error('DataTable init (.datatable) a échoué, le reste de la page continue :',e);}
